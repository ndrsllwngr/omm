...
- `docker run -p 80:3000 omm:latest` - Runs docker image with name `omm` and tag `latest` and maps container port `3000` to port `80` (go to http://localhost)
...
- `docker compose up` - Use `docker compose` to build & run the application (on http://localhost) - **recommended for development**
